...
sgpp::base::AsReductionResult reduceBorehole() {
  ...
  sgpp::base::DistributionsVector dist(types, chars, boreholeBb);
  ...
  //configs[8] = c1;
  //configs[9] = c1;
  //sgpp::base::DimReduction::reduceASRandom(unitFunc, dist, config, 2);
  sgpp::base::AsReductionResult result =
      sgpp::base::DimReduction::reduceAS(unitFunc, dist, samples, configs);
  return result;
}
  ...
  sgpp::base::AsReductionResult reduceAnalytical()
{
  size_t dims = 10;
    std::shared_ptr<sgpp::base::ScalarFunction> func =
        std::make_shared<sgpp::base::WrapperScalarFunction>(dims, gfunction);
...
int main()
{
  ...
  mat.transpose();
  mat.toFile("ebola-d3-l2-0.5.txt");
}
