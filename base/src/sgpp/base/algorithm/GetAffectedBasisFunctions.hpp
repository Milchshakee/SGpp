...
/**
 * Template Specialization for AnovaBoundaryBasis basis.
 */
template <>
class GetAffectedBasisFunctions<AnovaBoundaryBasis<int, unsigned int> > {
  typedef AnovaBoundaryBasis<int, unsigned int> SAnovaBoundaryBase;
 ...
 public:
  ...
  void operator()(SAnovaBoundaryBase& basis, const DataVector& point,
                  std::vector<std::pair<size_t, double> >& result) {
    bool useBB = false;
 ...
 protected:
  ...
  void rec(SAnovaBoundaryBase& basis, const DataVector& point, size_t current_dim, double value,
    ...
    while (true) {
      ...
      if (storage.isInvalidSequenceNumber(seq)) {
        ...
        if (work_level == -1) {
          // handle constant function on level -1
          // level -1, index 0
  ...
  void recBB(SAnovaBoundaryBase& basis, const DataVector& point, size_t current_dim, double value,
             GridStorage::grid_iterator& working, std::vector<std::pair<size_t, double> >& result) {
    typedef level_t level_type;
    typedef index_t index_type;
